{ MftQueryUsn
  ...
  The file contains basic functionality for USN journal queries. The file's
  content is copy of unit from the source below with minor modifications for
  its use for x64 platform.
...
*
  The source reference of original unit:
** https://bitbucket.org/ZeljkoMarjanovic/delphi-mft-journal-helpers/src/a2040df226cb/Units/uMFT.pas?fileviewer=file-view-default
}
unit MftQueryUsn;
...
type
  ...
  WCHAR_PTR_ARR = record
    case Integer of
    0: (
      Ptr: PWChar
    );
    1: (
      Arr: array[0..0] of WChar
    );
  end;
  USN_RECORD = record
...
function GetRootHandle(Drive: String; Async: Boolean; NeedWriteAccess: Boolean): Cardinal;
...
function AllocMFTEnumBuffer(ARootHandle: Cardinal; bRaise : Boolean = true): Pointer;
procedure AdjustChangeJournal(ARootHandle: Cardinal);
function EnumMFTEntries(ARootHandle: Cardinal; AMFTEnumBuff: Pointer;
  EnumCallBack: TMFTEnumCallback; Extra: Pointer = nil): Boolean;
function GetNameFromRec(AUSN: PUSNRecord; UseOffset: Boolean = false): WideString;
function IsPartitionNTFS(Drive: String): Boolean;
...
begin
  ...
  Result.FileName.Ptr := PWChar(Integer(P) + Result.FileNameOffset)
end;
...
begin
  ...
  try
    ...
    begin
      PUSN := PUSNRecord(NativeUInt(P) + sizeof(Int64));
...
begin
  ...
  begin
    ...
    if not UseOffset then
     ...
     {$ELSE}
      CopyMemory(@Result[1], Pointer(NativeUInt(@AUSN.FileName) - 4),
        AUSN.FileNameLength)
     {$ENDIF}
    else
      CopyMemory(@Result[1], Pointer(NativeUInt(AUSN) + AUSN.FileNameOffset), AUSN.FileNameLength);
  end
end;
...
begin
  Result := False;
  try
    Handle := GetRootHandle(Drive, false, false);
  except
    Exit;
  end;
